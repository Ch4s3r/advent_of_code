...
fn main() {
    ...
    for ((junction1, junction2), _) in junctions_to_connect.iter() {
        ...
        if circuits.len() == junctions.len() && circuits.iter().map(|(_, value)| value).all_equal() {
            dbg!(
                "All junctions connected by now thanks to junctions: {:?} and {:?}",
                junction1,
                junction2
            );
            dbg!("Final circuit length: {:?}", junction1.x * junction2.x);
            break;
        }
    }
}
