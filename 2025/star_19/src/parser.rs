...
#[derive(Debug, Clone)]
pub struct Machine {
    pub indicator_light_diagram: Vec<bool>,
...
// Parse light diagram [...] and convert to indicator lights (Vec<bool>)
fn parse_light_diagram(input: &str) -> IResult<&str, Vec<bool>> {
    ...
    .parse(input)?;
    let lights = content.chars().map(|c| c == '#').collect();
    Ok((input, lights))
}
...
pub fn parse_line(input: &str) -> IResult<&str, Machine> {
    ...
    let (input, indicator_light_diagram) = parse_light_diagram(input)?;
