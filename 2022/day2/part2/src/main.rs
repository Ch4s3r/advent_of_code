...
fn parse_game(input: &str) -> IResult<&str, Game> {
    ...
    Ok((
        ...
        Game {
            ...
            game_outcome: GameOutcome::from_str(player).unwrap(),
        },
    ))
}
...
impl Game {
    fn calculate_points(&self) -> i32 {
        ...
        let player_points = i32::from(&self.get_player_move());
    ...
    fn get_player_move(&self) -> HandType {
        match self.game_outcome {
            Lose => previous_cycle(&self.opponent).unwrap(),
            Draw => self.opponent.clone(),
            Win => next_cycle(&self.opponent).unwrap(),
        }
    }
}
